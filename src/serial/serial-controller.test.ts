  ...
  describe("syncEsp", () => {
    it("should succeed on the first attempt", async () => {
      ...
      const sendCommandSpy = vi
        .spyOn(serialController, "sendCommand")
        .mockResolvedValue(0); // Mock a successful command

      // Act
      await serialController.syncEsp();

      // Assert
      expect(serialController.setSignals).toHaveBeenCalledTimes(2);
      expect(serialController.setSignals).toHaveBeenCalledWith({
        dataTerminalReady: false,
        requestToSend: true,
      });
      expect(serialController.setSignals).toHaveBeenCalledWith({
      ...
      expect(sendCommandSpy).toHaveBeenCalledWith(expect.any(EspCommandSync));
    });
    ...
    it("should succeed after a few retries", async () => {
      ...
      const sendCommandSpy = vi
        .spyOn(serialController, "sendCommand")
        .mockRejectedValueOnce(new Error("Sync failed")) // Fail first time
        .mockRejectedValueOnce(new Error("Sync failed")) // Fail second time
        .mockResolvedValue(0); // Succeed third time

      // Act
      await serialController.syncEsp();

      // Assert
      expect(serialController.setSignals).toHaveBeenCalledTimes(6); // 2 calls per attempt
      expect(sendCommandSpy).toHaveBeenCalledTimes(3);
      expect(sleep).toHaveBeenCalledTimes(2); // Called after each failure
    });
    ...
    it("should throw an error after all retries fail", async () => {
      ...
      // Act & Assert
      await expect(serialController.syncEsp(3)).rejects.toThrow(
        "Failed to sync with device after 3 attempts",
      );
    ...
    it("should reset signals even if sendCommand throws a non-standard error", async () => {
      // Arrange
      const sendCommandSpy = vi
        .spyOn(serialController, "sendCommand")
        .mockRejectedValue("Some weird error");

      // Act & Assert
      await expect(serialController.syncEsp(1)).rejects.toThrow(
        "Failed to sync with device after 1 attempts",
      );
